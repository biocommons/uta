...
class GeneAccessionsWriter(csv.DictWriter):
    def __init__(self,tsvfile):
        csv.DictWriter.__init__(self,tsvfile, fieldnames=GeneAccessions._fields, delimiter=b'\t', lineterminator="\n")
...
class GeneAccessionsReader(csv.DictReader):
    def __init__(self,tsvfile):
        ...
        if set(self.fieldnames) != set(GeneAccessions._fields):
            raise RuntimeError('Format error: expected header with these columns: '+','.join(GeneAccessions._fields) + " but got: "+','.join(self.fieldnames))
...
if __name__ == '__main__':
    ...
    with open(tmpfn,'w') as f:
        ...
        for i in range(3):
            es = GeneAccessions(**dict([(k,k+":"+str(i)) for k in GeneAccessions._fields]))
            esw.write(es)
