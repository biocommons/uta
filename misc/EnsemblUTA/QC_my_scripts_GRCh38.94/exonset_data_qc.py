...
Workflow
exonset data structure
tx_ac	            alt_ac	    method	    strand	exons_se_i
ENST00000599728.1	NC_000019.9	genebuild	1	    58859118,58859210;58864686,58864827;58865079,58865405

Intended dictionary structure
{ 'ENST00000599728.1':
    [NC_000019.9,	genebuild,	1,  58859118,58859210;58864686,58864827;58865079,58865405]
}
"""
...
# Open and read data
query_file = os.path.join(QUERY, 'exonset')
reference_file = os.path.join(REFERENCE, 'exonset_90')
...
for ref_set in reference_list:
    if re.match('#', ref_set):
        ...
        reference_dict[ref_set[0]] = ref_set[1:]
...
for que_set in query_list:
    if re.match('#', que_set):
        ...
        query_dict[que_set[0]] = que_set[1:]
...
result_summary = 'Results summary\n%s query records match the reference dataset\n%s query ' \
    'records do not match the reference dataset\n%s records in total were compared\n%s records were in the ref data set\n%s records were in the query data set\n\n' % (str(pass_count),
                                                                                                                                                                       ...
                                                                                                                                                                       str(len(query_dict.keys()))                                                                                                                                )# Write results data
fails_handle = os.path.join(RESULTS, 'exonset_fail.txt')
fail_me = open(fails_handle, 'w')
passes_handle = os.path.join(RESULTS, 'exonset_pass.txt')
