...
ensembl-90	ENST00000370535	LINC00632	    30,342	    0,39;39,75;75,1381
...
# Open and read data
query_file = os.path.join(QUERY, 'txinfo')
reference_file = os.path.join(REFERENCE, 'merged_txinfo')
...
for ref_set in reference_list:
    if re.match('#', ref_set):
        ...
        try:
            ...
            reference_dict[ref_set[1]] = [ref_set[0]] + ref_set[2:]
        except IndexError:
            continue
...
for que_set in query_list:
    if re.match('#', que_set):
        ...
        try:
            ...
            query_dict[que_set[1].split('.')[0]] = [que_set[0]] + que_set[2:]
        except IndexError:
            continue
...
# Write results data
fails_handle = os.path.join(RESULTS, 'txinfo_fail.txt')
fail_me = open(fails_handle, 'w')
passes_handle = os.path.join(RESULTS, 'txinfo_pass.txt')
